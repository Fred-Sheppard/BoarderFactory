...
public class Testing {
    ...
    private static void drawPlane(SeatGui[][] seats, AisleGui aisle, 
        ...
        System.out.println("Airplane Boarding Simulation\n");
        
        // Draw waiting line first
        System.out.println("Waiting Line: (" + waitingCount + " passengers)");
        // Draw waiting passengers with their destination colors
        for (int i = 0; i < waitingCount; i++) {
            if (i == 0 && person != null) {
                ...
                PersonGui waitingPerson = new ModernPersonGui();
        ...
        for (int row = 0; row < 6; row++) {
            ...
            if (row == personRow && personCol == 2) {
                person.paint();  // Person will be colored based on their destination
            } else {
                aisle.paint();
            }
            System.out.print("    ");
        ...
        System.out.println("\nLegend: S=Empty Seat, P=Person/Occupied Seat");
        System.out.println("Colors: Blue=Window Seats, Green=Middle Seats, Red=Aisle Seats");
    }
} 